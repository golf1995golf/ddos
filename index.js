...
async function poptto() {
    if (process.argv.length !== 6) {
        console.log("node poptto.js https://betflix-amb.com/login 1000 39 off");
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'off') {
            ...
            console.log("PROXY")
            const proxyscrape = await axios.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=1000000&country=all&ssl=all&anonymity=all');
            //const proxyscrape = await axios.get('http://127.0.0.1:3333/pp');
